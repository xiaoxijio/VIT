...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # (1, 1, hidden_size) -> (b, 1, hidden_size)
        if self.hybrid:
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for _ in range(config.transformer["num_layers"]):  # 构造多个 Transformer 层
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        ...
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():  # 如果当前模型的位置嵌入尺寸与预训练模型一致，直接复制
